...
import { executeGraphTool } from "./execute.js";
...
async function handleRequest(body: {
  ...
  if (body.method === "initialize") {
    return jsonRpc(id, {
      ...
      capabilities: { tools: {} },
      serverInfo: { name: "graph-the-graph", version: "0.1.0" },
    });
  }
  ...
  if (body.method === "tools/call") {
    ...
    const result = await executeGraphTool(name, args, credentials);
...
async function main() {
  ...
  if (!raw.trim()) {
    process.stdout.write(
      jsonRpcError(null, -32_600, "Empty MCP request body")
    );
    return;
  }
  let body: {
