...
Portable Hermes agent plugin for The Graph protocol tools (`graph_*`). Includes an MCP stdio server via `@bleyle823/graph-core`.
...
The MCP server runs `graph-mcp` from `@bleyle823/graph-core`. Configure env vars in `~/.hermes/config.yaml` under `mcp_servers.the-graph`:
