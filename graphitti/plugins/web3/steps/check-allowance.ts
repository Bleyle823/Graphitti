...
async function stepHandler(
  ...
  let tokenAddress: string | null = input.tokenAddress ?? null;
  if (!tokenAddress) {
    const config =
      typeof input.tokenConfig === "string"
        ? input.tokenConfig
        : JSON.stringify(input.tokenConfig);
    if (config.startsWith("0x")) {
      try {
        tokenAddress = parseTokenAddress(config);
      } catch {
        tokenAddress = null;
      }
    } else {
      try {
        const parsed = JSON.parse(config) as { customToken?: { address?: string } };
        if (parsed.customToken?.address) {
          tokenAddress = parseTokenAddress(parsed.customToken.address);
        }
      } catch {
        tokenAddress = null;
      }
  ...
  try {
    const [allowanceRaw, decimals, symbol] = await rpcManager.executeWithFailover(
      (provider) => {
