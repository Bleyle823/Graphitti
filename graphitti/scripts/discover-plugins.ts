...
let registeredProtocolEntries: Array<{
  slug: string;
  fileStem: string;
...
async function loadProtocolDefinitions(): Promise<
  Array<{
    slug: string;
    fileStem: string;
    definition: import("@/lib/protocol-registry").ProtocolDefinition;
...
async function registerProtocolPlugins(): Promise<string[]> {
  const { protocolToPlugin, registerProtocol } = await import(
  ...
  for (const entry of definitions) {
    ...
    if (!entry.definition.hubOnly) {
      registerIntegration(protocolToPlugin(entry.definition));
    }
...
`;
  ...
  console.log(
    `Generated protocols/index.ts with ${registeredProtocolEntries.length} protocol(s)`
  );
}
...
async function generateStepRegistry(): Promise<void> {
  ...
  const importerEntries = stepEntries
    .flatMap(({ actionId, integration, stepImportPath, stepFunction }) => {
      ...
      const importPath = isProtocolStep
        ? `@/plugins/protocol/steps/${stepImportPath}`
...
async function main(): Promise<void> {
  ...
  // Register protocols BEFORE importing plugins so plugins that inject actions
  // into protocol integrations (e.g. cowswap, safe) find them at import time.
