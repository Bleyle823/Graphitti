...
- **Arc USDC**: Marketplace settlement asset on Arc (`0x3600…0000`, 6-decimal ERC-20). Native Arc gas USDC is 18 decimals. Paid listing calls settle as Circle Gateway nanopayments over x402 (`GatewayWalletBatched`) on Arc mainnet (`eip155:5042`) unless the listing chain is Arc Testnet.
...
Document Browser, Agent (MCP), API, and **Graphitti org treasury on Privy** paths. Do not document Turnkey, Tempo/MPP, Base x402, CLI, keepers/scheduler, or protocol plugins that are not shipped.
